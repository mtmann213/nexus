...
## Subject: Multi-Agent Systems and the Evaluation Layer
...
3. **Manage** complex conversation state across multiple independent AI agents.
...
To make agents "talk" to each other, the Orchestrator must act as the **Dispatcher**. 
* **The Solution:** We learned to build "Clean History" buffers for each agent to satisfy the strict alternating **User -> Assistant** requirements of local servers.
...
* **The Analogy:** In RF, this is **Verification & Validation (V&V)**. You don't just build a filter; you measure its S-parameters against the design spec to prove it works.
...
| **Eval (Evaluation)** | A systematic way to measure the performance or accuracy of an AI agent. | **V&V Testing** or Performance Characterization. |
| **Ground Truth** | The "correct" answer used as a baseline for evaluation. | A **Calibrated Reference** standard. |
| **The Judge** | A high-level model used to score the performance of other models. | A **Third-Party Auditor**. |
| **Regression** | When a new change causes the AI to perform worse than before. | **Link Budget Degradation** after a change. |
...
3. How do we prevent a "Regression" when updating an agent's system prompt?
...
* **Key Feature:** This script correctly alternates roles (User/Assistant) to satisfy strict local LLM templates.
